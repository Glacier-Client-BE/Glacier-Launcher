...
object GPlayAPI {
    ...
    /** Non-throwing variant of [getAuthData] for callers (PlayStoreValidator) that
     *  treat "no account signed in" as a normal, reportable outcome rather than
     *  an exception to catch. */
    fun getAuthDataOrNull(context: Context): AuthData? =
        try { getAuthData(context) } catch (e: IllegalArgumentException) { null }
